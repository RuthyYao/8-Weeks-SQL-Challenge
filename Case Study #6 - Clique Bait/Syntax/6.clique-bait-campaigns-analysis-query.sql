...
-- Calculate the number of users who didn't receive impressions
	-- Note: We can't use impression = 0 as the condition to filter the data because a user could have visited the website for twice during the campaign but recieved the impression only once. In this case, when you count the users using the condition of impression is 0, this customer will be incorrectly included in this group.
